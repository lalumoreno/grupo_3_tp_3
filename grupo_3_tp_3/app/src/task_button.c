...
static void button_process_state(bool is_pressed, button_event *event);
...
static void button_task(void *argument)
{
	...
	while (1)
	{
		...
		if (bnt_event != NULL)
		{
			...
			button_process_state(is_pressed, bnt_event);
			...
			if (bnt_event->type != BUTTON_TYPE_NONE)
			{
			...
			{
				vPortFree(bnt_event); // Liberar si no se generó evento
									  // uart_log("BTN - Memoria de bnt_event liberada \r\n");
			}
		}
		else
		{
			uart_log("BTN - Error al alocar memoria para bnt_event\r\n");
		}
